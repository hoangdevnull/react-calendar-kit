...
Styling can be done via `classNames` or `styles` props on each components.

Below is example for styling the calendar using tailwindcss and cn (`clsx`)

## Date Input

<CK.DateInput className="mx-auto mt-4" />
...
<CK.TimeInput className="mx-auto mt-4" />
...
export const DateRange = () => {
  const { ref,  getDateRangeProps, } = useDateRangePicker();

return <CK.DateRangeInput {...getDateRangeProps} ref={ref} className="mt-4" />;
};

<DateRange />

```tsx copy
import { Date RangeInput } from 'react-calendar-kit';
...
<div className="flex flex-wrap gap-6 mt-4">
  <CK.Calendar />
  <CK.Calendar withPicker />
  <CK.Calendar headerLayout="left" />
  <CK.Calendar headerLayout="right" />
</div>
...
<Calendar
  classNames={{
    ...
    gridBodyRow: 'flex justify-center items-center data-[picker-expanded=true]:pointer-events-none pointer-events-auto',
...
<CK.RangeCalendar className="mt-4" />

```tsx copy
import { RangeCalendar } from 'react-calendar-kit';
